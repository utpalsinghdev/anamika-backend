import { Controller, Get, Post, Body, Patch, Param, Delete, UploadedFile, UseInterceptors, HttpCode, Res } from '@nestjs/common';
...
export class AgentApplicationController {
  constructor(private readonly agentApplicationService: AgentApplicationService) { }

  @Post()
  @HttpCode(200)
  @UseInterceptors(FileInterceptor('resume'))
  async create(@UploadedFile() file, @Body() createAgentApplicationDto: CreateAgentApplicationDto, @Res({ passthrough: true }) res: Response) {
    try {
      const data = await this.agentApplicationService.create(createAgentApplicationDto);
      return {
        success: true,
